...
const { VERIFICATION_STATUS, DOCUMENT_STATUS } = require('../../src/config/constants');
...
describe('Verification Service Unit Tests', () => {
  ...
  describe('verifyByQRCode', () => {
    ...
    it('should return valid verification and log attempt for active non-expired document', async () => {
      prisma.document.findUnique.mockResolvedValue(mockActiveDoc);
      prisma.verificationLog.create.mockResolvedValue({ id: 'log-1' });
    ...
    it('should return revoked status and log attempt when document status is revoked', async () => {
      ...
      prisma.document.findUnique.mockResolvedValue(mockRevokedDoc);
      prisma.verificationLog.create.mockResolvedValue({ id: 'log-2' });
    ...
    it('should return expired status and log attempt when document expiryDate has passed', async () => {
      ...
      prisma.document.findUnique.mockResolvedValue(mockExpiredDoc);
      prisma.verificationLog.create.mockResolvedValue({ id: 'log-3' });
  ...
  describe('getVerificationLogs', () => {
    it('should return paginated verification logs', async () => {
      ...
      prisma.verificationLog.findMany.mockResolvedValue(mockLogs);
      prisma.verificationLog.count.mockResolvedValue(1);
